"""
Pass 2 (Monthly Prediction, Non-Overlapping Targets) — NO LOOK-AHEAD VERSION:
Predict next-month (21-day) cumulative idiosyncratic return using
4 weekly-averaged PC1 lags. Train ONLY on non-overlapping monthly
observations (every 21st day).

Look-ahead fix: training candidates are restricted to indices i where
  i < t - FORWARD_DAYS
so that y_monthly[i] = y[i+1:i+22].sum() does not include any return
on or after day t.

Survivorship fix: removed the `if np.isnan(y_monthly[t]): continue`
gate that conditioned signal output on knowing the future return.

Non-overlapping = clean standard errors, ~144 training obs per regression.

Expects: full_results_naics_EWMA/pc_scores_ewma.parquet
Saves:   signal + diagnostics to nolookahead/full_results_monthly_nonoverlapping/
"""
import os
import numpy as np
import polars as pl
...
DAYS_PER_WEEK  = 5
FORWARD_DAYS   = 21
STEP_SIZE      = 21    # only use every 21st day for training
REG_WINDOW     = 252   # in trading days (but only ~12 non-overlapping obs per year)
DATE_COL       = 'date'
ID_COL         = 'cusip'
...
RESULTS_DIR    = '/home/bpinoli/sub-industry-correlation/src/signal/naics_strats/EWMA_2_pass/nolookahead/full_results_monthly_nonoverlapping'


def _run_monthly_nonoverlapping(
    ...
    # ── Identify non-overlapping training dates ───────────────
    # Starting from the first valid date, take every 21st day
    valid_start = total_lookback
    non_overlap_idx = list(range(valid_start, n - forward_days, step_size))

    cusip_val = cusip_scores['cusip'][0]
    results_date, results_signal = [], []
    min_train = n_weekly_lags + 5  # fewer obs available, relax minimum
    ...
    for t in range(total_lookback, n - forward_days):
        ...
        # ── No-look-ahead training candidates ──────────────────
        # y_monthly[i] = y[i+1:i+22].sum()
        # Safe at time t iff i + forward_days < t  →  i < t - forward_days
        window_start = max(0, t - reg_window)
        train_candidates = [idx for idx in non_overlap_idx
                            if window_start <= idx < t - forward_days]

        # Filter to valid rows
        train_idx = [idx for idx in train_candidates
                     if not np.isnan(lagged_weekly[idx]).any()
                     and not np.isnan(y_monthly[idx])]
        ...
        # ── Survivorship fix ────────────────────────────────────
        # Original code: if np.isnan(y_monthly[t]): continue
        # This conditioned signal output on knowing the future return (look-ahead).
        # Removed: we emit the signal regardless of whether the future return
...
def save_diagnostics(signals_df: pl.DataFrame, pc_df: pl.DataFrame, results_dir: str):
    ...
    for cusip in signals_df['cusip'].unique().to_list():
        cdf = pc_df.filter(pl.col('cusip') == cusip).sort('date')
    ...
    axes[0].set_title('Daily Cross-Sectional Rank IC — Monthly Non-Overlapping (No Look-Ahead)')
    axes[0].legend()
    ...
    ax.set_title('Signal Distribution — Monthly Non-Overlapping (No Look-Ahead)')
    plt.tight_layout()
...
def main():
    ...
    output_path = os.path.join(project_root, 'data/signal_monthly_nonoverlapping_nolook.parquet')
    ...
    total = len(cusips)
    print(f"\nRunning monthly non-overlapping lag regression (NO LOOK-AHEAD) for {total} stocks...")
    print(f"  {N_WEEKLY_LAGS} weekly lags, {FORWARD_DAYS}-day forward return")
    print(f"  Training on every {STEP_SIZE}th day only, capped at t - {FORWARD_DAYS}")
    print(f"  {N_WEEKLY_LAGS + 1} parameters, OLS window: {REG_WINDOW} days (~{REG_WINDOW // STEP_SIZE} training obs)")

    all_signals = []
    for i, cusip in enumerate(cusips):
        cusip_data = pc_df.filter(pl.col('cusip') == cusip)
        try:
            sig_df = _run_monthly_nonoverlapping(cusip_data)
            if sig_df.height > 0:
    ...
    print(f"\n{'='*60}")
    print(f"COMPLETE — Monthly Non-Overlapping Signal (No Look-Ahead)")
