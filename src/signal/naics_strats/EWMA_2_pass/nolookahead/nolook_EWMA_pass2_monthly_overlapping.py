"""
Pass 2 (Monthly Prediction, Overlapping Targets) — NO LOOK-AHEAD VERSION:
...
Look-ahead fix: training window ends at t - FORWARD_DAYS so that no
training target y_monthly[i] overlaps with day t's returns.
  y_monthly[i] = y[i+1 : i+22].sum()
  Safe at time t iff i+21 < t, i.e. i < t - forward_days.
...
Saves:   signal + diagnostics to nolookahead/full_results_monthly_overlapping/
"""
import os
import numpy as np
import polars as pl
...
RESULTS_DIR    = '/home/bpinoli/sub-industry-correlation/src/signal/naics_strats/EWMA_2_pass/nolookahead/full_results_monthly_overlapping'
...
def _run_monthly_overlapping(
    ...
    for t in range(total_lookback, n - forward_days):
        ...
        # ── No-look-ahead training window ──────────────────────
        # y_monthly[i] = y[i+1:i+22].sum()
        # Safe at time t iff i + forward_days < t  →  i < t - forward_days
        train_end = t - forward_days
        window_start = max(0, t - reg_window)
        if train_end <= window_start:
            continue

        valid_mask = (~np.isnan(lagged_weekly[window_start:train_end]).any(axis=1)
                      & ~np.isnan(y_monthly[window_start:train_end]))
...
def save_diagnostics(signals_df: pl.DataFrame, pc_df: pl.DataFrame, results_dir: str):
    ...
    axes[0].set_title('Daily Cross-Sectional Rank IC — Monthly Overlapping (No Look-Ahead)')
    axes[0].legend()
    ...
    ax.set_title('Signal Distribution — Monthly Overlapping (No Look-Ahead)')
    plt.tight_layout()
...
def main():
    ...
    output_path = os.path.join(project_root, 'data/signal_monthly_overlapping_nolook.parquet')
    ...
    total = len(cusips)
    print(f"\nRunning monthly overlapping lag regression (NO LOOK-AHEAD) for {total} stocks...")
    print(f"  {N_WEEKLY_LAGS} weekly lags, {FORWARD_DAYS}-day forward return")
    print(f"  Training window ends at t - {FORWARD_DAYS} to prevent target overlap")
    ...
    print(f"\n{'='*60}")
    print(f"COMPLETE — Monthly Overlapping Signal (No Look-Ahead)")
