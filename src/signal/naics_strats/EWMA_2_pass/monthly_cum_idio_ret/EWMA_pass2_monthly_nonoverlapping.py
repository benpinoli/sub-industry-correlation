"""
Pass 2 (Monthly Prediction, Non-Overlapping Targets):
...
Saves:   signal + diagnostics to full_results_monthly_nonoverlapping/
"""
import os
import numpy as np
import polars as pl
...
RESULTS_DIR    = '/home/bpinoli/sub-industry-correlation/src/signal/full_results_monthly_nonoverlapping'
...
def _run_monthly_nonoverlapping(
    ...
    for t in range(total_lookback, n - forward_days):
        ...
        # Training set: non-overlapping observations within rolling window
        window_start = max(0, t - reg_window)
        train_candidates = [idx for idx in non_overlap_idx
                            if window_start <= idx < t]
...
def save_diagnostics(signals_df: pl.DataFrame, pc_df: pl.DataFrame, results_dir: str):
    ...
    axes[0].set_title('Daily Cross-Sectional Rank IC — Monthly Non-Overlapping')
    axes[0].legend()
    ...
    ax.set_title('Signal Distribution — Monthly Non-Overlapping')
    plt.tight_layout()
...
def main():
    ...
    output_path = os.path.join(project_root, 'data/signal_monthly_nonoverlapping.parquet')
    ...
    total = len(cusips)
    print(f"\nRunning monthly non-overlapping lag regression for {total} stocks...")
    print(f"  {N_WEEKLY_LAGS} weekly lags, {FORWARD_DAYS}-day forward return")
    print(f"  Training on every {STEP_SIZE}th day only")
    ...
    print(f"\n{'='*60}")
    print(f"COMPLETE — Monthly Non-Overlapping Signal")
