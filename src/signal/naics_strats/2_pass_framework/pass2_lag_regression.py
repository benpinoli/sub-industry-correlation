"""
Pass 2: Load precomputed PC1 scores from Pass 1, run distributed lag
regressions for each stock, and save the signal parquet.
This is fast (~30-60 min) because no PCA/SVD is computed.
"""
import os
import numpy as np
import polars as pl
...
PC_SCORES_PATH = '/home/bpinoli/sub-industry-correlation/src/signal/full_results_naics/pc_scores.parquet'
...
def _run_lag_regression(
    ...
    PC1 features and run rolling OLS to predict next-day idio return.
    """
    # Sort by date, extract arrays
    ...
    # Rolling regression: 252-day window, predict t+1
    ...
    for t in range(min_train, n - 1):
        ...
        # Use only the last 252 days for training
...
def main():
    ...
    # ── 1. Load precomputed PC scores ─────────────────────────
    print(f"Loading PC scores from {PC_SCORES_PATH}...")
    ...
    print(f"\n{'='*60}")
    print(f"PASS 2 COMPLETE — Signal Generation (NAICS, Distributed Lag)")
